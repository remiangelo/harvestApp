-- Create safety analysis tables for AI red flag detection (SAFE VERSION - checks for existing objects)
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'safety_analyses_conversation_user_key') THEN
    ALTER TABLE safety_analyses ADD CONSTRAINT safety_analyses_conversation_user_key UNIQUE (conversation_id, user_id, created_at);
  END IF;
END $$;

-- Create indexes for faster queries (with IF NOT EXISTS checks)
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_safety_analyses_user_id') THEN
    CREATE INDEX idx_safety_analyses_user_id ON safety_analyses(user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_safety_analyses_match_id') THEN
    CREATE INDEX idx_safety_analyses_match_id ON safety_analyses(match_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_safety_analyses_conversation_id') THEN
    CREATE INDEX idx_safety_analyses_conversation_id ON safety_analyses(conversation_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_safety_analyses_created_at') THEN
    CREATE INDEX idx_safety_analyses_created_at ON safety_analyses(created_at DESC);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_safety_analyses_safety_score') THEN
...
-- Create indexes for red flag reports (with IF NOT EXISTS checks)
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_red_flag_reports_reporter_id') THEN
    CREATE INDEX idx_red_flag_reports_reporter_id ON red_flag_reports(reporter_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_red_flag_reports_reported_user_id') THEN
    CREATE INDEX idx_red_flag_reports_reported_user_id ON red_flag_reports(reported_user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_red_flag_reports_conversation_id') THEN
    CREATE INDEX idx_red_flag_reports_conversation_id ON red_flag_reports(conversation_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_red_flag_reports_reviewed') THEN
    CREATE INDEX idx_red_flag_reports_reviewed ON red_flag_reports(reviewed);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_red_flag_reports_severity') THEN
    CREATE INDEX idx_red_flag_reports_severity ON red_flag_reports(severity);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_red_flag_reports_created_at') THEN
...
-- Create index for user safety settings (with IF NOT EXISTS check)
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_user_safety_settings_updated_at') THEN
...
-- Create indexes for ready to move checks (with IF NOT EXISTS checks)
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_ready_to_move_checks_user_id') THEN
    CREATE INDEX idx_ready_to_move_checks_user_id ON ready_to_move_checks(user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_ready_to_move_checks_conversation_id') THEN
    CREATE INDEX idx_ready_to_move_checks_conversation_id ON ready_to_move_checks(conversation_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_ready_to_move_checks_checked_at') THEN
    CREATE INDEX idx_ready_to_move_checks_checked_at ON ready_to_move_checks(checked_at DESC);
  END IF;
END $$;

-- Safety metrics aggregation (for dashboard) - DROP and RECREATE to ensure it's current
DROP VIEW IF EXISTS user_safety_metrics;

CREATE VIEW user_safety_metrics AS
SELECT 
  u.user_id,
...
DROP TRIGGER IF EXISTS update_user_safety_settings_timestamp ON user_safety_settings;
...
BEGIN
  ...
  DROP POLICY IF EXISTS "Users can view their own safety analyses" ON safety_analyses;
  DROP POLICY IF EXISTS "System can insert safety analyses" ON safety_analyses;
  
  -- Policies for red_flag_reports
  DROP POLICY IF EXISTS "Users can create red flag reports" ON red_flag_reports;
  DROP POLICY IF EXISTS "Users can view reports they created" ON red_flag_reports;
  
  -- Policies for user_safety_settings
  DROP POLICY IF EXISTS "Users can view their own safety settings" ON user_safety_settings;
  DROP POLICY IF EXISTS "Users can update their own safety settings" ON user_safety_settings;
  DROP POLICY IF EXISTS "Users can insert their own safety settings" ON user_safety_settings;
  
  -- Policies for ready_to_move_checks
  DROP POLICY IF EXISTS "Users can view their own move checks" ON ready_to_move_checks;
  DROP POLICY IF EXISTS "System can insert move checks" ON ready_to_move_checks;
END $$;

-- Recreate policies for safety_analyses
...
-- Recreate policies for red_flag_reports
...
-- Recreate policies for user_safety_settings
...
-- Recreate policies for ready_to_move_checks
...
-- Grant permissions for authenticated users (REVOKE first to avoid conflicts)
DO $$
BEGIN
  REVOKE ALL ON safety_analyses FROM authenticated;
  REVOKE ALL ON red_flag_reports FROM authenticated;
  REVOKE ALL ON user_safety_settings FROM authenticated;
  REVOKE ALL ON ready_to_move_checks FROM authenticated;
  REVOKE ALL ON user_safety_metrics FROM authenticated;
...
DO $$
BEGIN
  RAISE NOTICE 'Safety tables migration completed successfully!';
END $$;